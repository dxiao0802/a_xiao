...
- 大西庫存管理系統 - Dashboard 原型
- Tech Stack: React + Recharts + Tailwind (CDN classes)
- 
- 架構說明：
- - 本檔案為純前端 Demo，資料以 mock state 模擬
- - 實際部署時，將 MOCK_DATA 替換為 axios/fetch API 呼叫
- - 移植性：所有 API 呼叫集中在 /src/services/api.js，方便 Capacitor 打包
    */
...
{/* Google Fonts */}
<style>{`@import url('https://fonts.googleapis.com/css2?family=IBM+Plex+Sans:wght@400;500;600;700&family=DM+Mono:wght@400;500&family=Noto+Sans+TC:wght@400;500;700&display=swap'); * { box-sizing: border-box; } ::-webkit-scrollbar { width: 6px; } ::-webkit-scrollbar-track { background: transparent; } ::-webkit-scrollbar-thumb { background: rgba(255,255,255,0.1); border-radius: 3px; } input, select { font-family: inherit; } select option { background: #1a1f2e; color: #f1f5f9; } @keyframes pulse-ring { 0%,100%{opacity:1} 50%{opacity:0.5} }`}</style>
